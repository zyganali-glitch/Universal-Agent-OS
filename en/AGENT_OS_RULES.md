...
### 0.1) Master Integrity Lock (IL) Registry

This donor expects the target repo plan template and adapters to carry these locks verbatim or with repo-language equivalents:
...
28. **IL-13: Dynamic README & Live Documentation Lock (Live-Docs Sync):** 
    - **Master README:** The exact second the Phase-0 Consensus is complete, the Agent MUST automatically generate a comprehensive `README.md` file at the root level serving as the project's master showcase. Whenever a feature, installation step, or architecture changes, this Master `README.md` MUST be updated instantly (Live-Sync) along with the task tables!
    - **Other Files:** `PROJECT_STRUCTURE.md` and `USER_GUIDE.md` must reflect the CURRENT, live state of the project. Living ecosystem files like `TECH_DEBT_AND_SECURITY.md`, `BUSINESS_MODEL.md` evolve alongside the core codebase.
29. **IL-14: Adapter Currency Lock:** As the very first action upon entering this repository, the agent MUST inspect its own native root adapter file. If the agent's platform has evolved to support a more modern or advanced instruction-loading format than what is currently in the adapter, the agent MUST self-update the adapter file to the latest native standard before executing any code changes.
30. **IL-15: Master Roadmap & Coding Lock:** After Phase-0, the agent MUST create a project-specific global plan template and a "Plan Creation Plan" (Master Roadmap). The Master Roadmap is a living document tracking all end-to-end child plans. **NO CODING OR EXECUTION CAN START** until all child plans listed in the Master Roadmap are drafted and prioritized.
31. **IL-16: Continuous Sync & Collective Memory Lock:** At the end of EVERY executed task, the agent MUST synchronously update the project's Collective Memory (`AGENT_MEMORY_AND_LESSONS.md`, `AGENT_ARCHITECTURE_AND_PATTERNS.md`, `AGENT_ENVIRONMENT_AND_API.md`, `AGENT_USER_PREFERENCES.md`), the user repository's README, and any other relevant documentation. Keeping the living documentation updated is a hard requirement for marking a task as DONE. Agents make mistakes, but they MUST NEVER make the same mistake twice. Any lessons learned must be logged immediately.
