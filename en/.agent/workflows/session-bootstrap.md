...
The agent MUST ask these coherent and logical questions **STRICTLY ONE BY ONE**. Do NOT list all questions at once in a single message. You MUST wait for the user to answer the first question before moving to the next. **The agent must offer its own justified recommendations, but always surrender the final decision to the user.**
